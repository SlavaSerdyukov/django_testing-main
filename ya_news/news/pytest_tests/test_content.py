...
def test_news_sorted_by_freshness(client, news_home_url, list_news):
    """
    Проверяет, что новости на главной странице отсортированы от самой свежей
    к самой старой. Свежие новости находятся в начале списка.
    """
    response = client.get(news_home_url)
    assert 'object_list' in response.context
    all_dates = [
...
def test_comments_sorted_chronologically(
    ...
    """
    Проверяет, что комментарии на странице новости отсортированы в
    хронологическом порядке: старые в начале списка, новые — в конце.
    """
    response = client.get(news_detail_url)
    all_dates = [
        comment.created for comment in response.context['news'].comment_set.all()
    ]
...
def test_authorized_client_has_form(client_reader, news_detail_url):
    """
    Проверяет, что авторизованный клиент видит форму для добавления
    комментария на странице новости.
    """
